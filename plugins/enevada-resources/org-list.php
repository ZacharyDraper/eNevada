...
class en_Organizations_List_Table extends WP_List_Table{
	...
	function prepare_items(){
	  ...
	  $per_page = 20;
	  $total_items = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}en_orgs WHERE status != 'trash';");
	  ...
	  // determine sorting
	  $orderby = filter_has_var(INPUT_GET, 'orderby') ? filter_input(INPUT_GET, 'orderby', FILTER_SANITIZE_STRING) : 'o.name';
	  ...
	  $this->items = $wpdb->get_results("SELECT o.id, o.status, o.name, o.created, u.display_name FROM {$wpdb->prefix}en_orgs AS o LEFT JOIN {$wpdb->prefix}users AS u ON o.created_by = u.ID WHERE o.status != 'trash' ORDER BY $orderby $order LIMIT $start_at, $per_page;");
	}
}
?>
<style type="text/css">
...
$table = new en_Organizations_List_Table();
$table->prepare_items(); 
$table->display();
?>