...
class en_Categories_List_Table extends WP_List_Table{
	...
	function prepare_items(){
	  ...
	  $per_page = 20;
	  $total_items = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}en_categories WHERE status != 'trash';");
	  ...
	  // determine sorting
	  $orderby = filter_has_var(INPUT_GET, 'orderby') ? filter_input(INPUT_GET, 'orderby', FILTER_SANITIZE_STRING) : 'c.name';
	  ...
	  $this->items = $wpdb->get_results("SELECT c.id, c.status, c.name, c.created, u.display_name FROM {$wpdb->prefix}en_categories AS c LEFT JOIN {$wpdb->prefix}users AS u ON c.created_by = u.ID WHERE c.status != 'trash' ORDER BY $orderby $order LIMIT $start_at, $per_page;");
	}
}
?>
<style type="text/css">
...
$table = new en_Categories_List_Table();
$table->prepare_items(); 
$table->display();
?>